...
type GateHandler struct {
	DB           *db.DB
	GatesService *services.GatesService
}

// NewSensorHandler creates a new SensorHandler
func NewGateHandler(db *db.DB, temperatureService *services.GatesService) *GateHandler {
	return &GateHandler{
		DB:           db,
		GatesService: temperatureService,
	}
}

// RegisterRoutes registers the sensor routes
func (h *GateHandler) RegisterRoutes(router *gin.RouterGroup) {
	gates := router.Group("/gates")
	{
		gates.GET("", h.GetSensors)
		gates.GET("/:id", h.GetSensorByID)
		gates.POST("", h.CreateSensor)
		gates.PUT("/:id", h.UpdateSensor)
		gates.DELETE("/:id", h.DeleteSensor)
		gates.PATCH("/:id/value", h.UpdateSensorValue)
	}
}

// GetSensors handles GET /api/v1/sensors
func (h *GateHandler) GetSensors(c *gin.Context) {
	...
	for i, sensor := range sensors {
		if sensor.Type == models.Temperature {
			tempData, err := h.GatesService.GetTemperatureByID(fmt.Sprintf("%d", sensor.ID))
			if err == nil {
...
func (h *GateHandler) GetSensorByID(c *gin.Context) {
...
func (h *GateHandler) GetTemperatureByLocation(c *gin.Context) {
	...
	tempData, err := h.GatesService.GetTemperature(location)
	if err != nil {
...
func (h *GateHandler) CreateSensor(c *gin.Context) {
...
func (h *GateHandler) UpdateSensor(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid sensor ID"})
		return
...
func (h *GateHandler) DeleteSensor(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid sensor ID"})
		return
...
func (h *GateHandler) UpdateSensorValue(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid sensor ID"})
		return
